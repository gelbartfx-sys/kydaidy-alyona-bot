...
разбор бесплатный, цены программы звучат только на созвоне (решение Кая
13.09, ТЗ эфир-воркшоп; до этого было «первые 10 бесплатно, дальше 7 000 ₽»).
...
from vstrecha import kbd_zapis, ne_pustit
...
DNEVNIK_METKA = "Дневник отношений, неделя"   # первая строка автозаявки dnevnik.py


def _otvety(row) -> list:
    try:
        return json.loads(row["answers"] or "[]") if row else []
    except (ValueError, TypeError):
        return []


def voprosy_zadany(row) -> bool:
    """Заявка есть И три вопроса уже заданы. Автозаявка из дневника (решение
    Кая 29.08) — ещё не заявка с вопросами: их задаём после дневника, перед
    выбором времени (решение Кая 13.09)."""
    otv = _otvety(row)
    return bool(row) and not (otv[:1] == [DNEVNIK_METKA] and len(otv) <= 3)
...
async def show_intro(msg: Message, tg_id: int, source: str = ""):
    ...
    if voprosy_zadany(await razbor_get(tg_id)):
        await msg.answer(d.RAZBOR_ALREADY, parse_mode=None,
                         reply_markup=kbd_zapis())
        return
    if await ne_pustit(msg, tg_id, source):   # ТЗ 13.09: разбор — после дневника
        return
    await msg.answer(d.RAZBOR_INTRO, parse_mode=None, reply_markup=_intro_kbd())
    try:
...
async def cb_razbor_go(cb: CallbackQuery):
    ...
    if voprosy_zadany(await razbor_get(tg_id)):
        await cb.message.answer(d.RAZBOR_ALREADY, parse_mode=None,
                                reply_markup=kbd_zapis())
        return
    # Кнопка «Оставить заявку» живёт в старых сообщениях — проверка и здесь,
    # иначе закрытая дверь открывалась бы из истории чата.
    if await ne_pustit(cb.message, tg_id, "razbor_go"):
        return
...
async def collect(msg: Message):
    ...
    # Срез дневника из автозаявки не теряем: ответы ложатся после него.
    prezhnie = _otvety(await razbor_get(tg_id))
    dnevnik_chast = prezhnie if prezhnie[:1] == [DNEVNIK_METKA] else []
    await razbor_save(tg_id, username, json.dumps(dnevnik_chast + st["answers"],
                                                 ensure_ascii=False))
    ...
    try:
        ...
        for q, a in zip(d.RAZBOR_Q, st["answers"]):
            lines += [q, a, ""]
        lines.append(f"Заявок всего: {await razbor_count()}")
...
if __name__ == "__main__":
    ...
    assert "₽" not in d.RAZBOR_INTRO and "десять" not in d.RAZBOR_INTRO, \
        "цены в боте нет — только на созвоне (решение Кая 13.09)"
