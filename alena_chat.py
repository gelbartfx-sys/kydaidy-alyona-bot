...
async def _schedule_followups(tg_id: int):
    ...
    СНЯТО 28.08 (мандат Кая): серия вела на Клуб «Манифест» 990 ₽ — продукт
    закрытой воронки. Гейт стоит здесь, в общем месте, а не у четырёх
    вызывающих: так ни один путь не поставит серию мимо. Вернуть можно
    одной строкой, когда тексты дожима будут написаны под новые продукты
    («Разбор сценария отношений», «Выйти из сценария»).
    """
    return
