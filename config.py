...
class Settings(BaseSettings):
    ...
    # 🔒 Продукты мёртвой воронки (мандат Кая 28.08: закрыта целиком). Цены
    # оставлены ТОЛЬКО для обратной классификации легаси-оплат в webhooks —
    # ни один оффер на них больше не строится, витрин в боте нет.
    # Витрины-продукты в Tribute деактивирует Кай (dashboard).
    ...
    # ── Эфир-воркшоп вместо цепочки 7 дней (решения Кая 13.09, efir.py) ──────
    # OFF по умолчанию: пока флаг выключен, бот байт-в-байт прежний — ни /efir,
    # ни ?start=efir, ни тика эфира, цепочка 7 дней с прежними кнопками.
    efir_enabled: bool = False
    # Комната эфира. Пусто — кнопка «Войти в эфир» не показывается, запись и
    # напоминания всё равно работают.
    efir_komnata_url: str = ""
